...
def RecursionSequence02():
    ...
    for i in range(2,k+1):
        a.append((p*a[i-1] + q*a[i-2]))  # 追加在数组a的后面
    print(a[k]%10000)
# RecursionSequence02()
...
def GradeSorted():
    ...
    # print(list)
    # d2=sorted(list,key = lambda item:(item[2],list.index(item)),reverse=False) # 未通过所有的测试用例...
    d2 = sorted(list, key=lambda item: (item[2], item[0],item[1]), reverse=False)
    # print(d2)
    for j in range(n):
